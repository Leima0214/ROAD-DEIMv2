...
cd /root/ROAD-DEIMv2-M1 || exit 90
ulimit -n 65535
mkdir -p logs

run_name="deimv2_n_japan4_m1_scqr_32e_seed42_20260814"
output_dir="outputs/${run_name}"
log_path="logs/${run_name}.log"
exit_path="logs/${run_name}.exitcode"

if [[ -e "${output_dir}" || -e "${log_path}" || -e "${exit_path}" ]]; then
  echo "M1_LAUNCH_REFUSED: target artifacts already exist" >&2
  exit 91
fi

CUDA_VISIBLE_DEVICES=0 /opt/conda/bin/python -u train.py \
  -c configs/deimv2/deimv2_hgnetv2_n_japan4_m1_scqr_32e.yml \
