...
cd /root/ROAD-DEIMv2-M4A || exit 90
ulimit -n 65535
mkdir -p logs reports

run_name="deimv2_n_japan4_m4a_dysample_p5p4_32e_seed42_20260814"
...
m4a_config="configs/deimv2/deimv2_hgnetv2_n_japan4_m4a_dysample_p5p4_32e.yml"

if [[ -e "${output_dir}" || -e "${log_path}" || -e "${exit_path}" \
      || -e "${preflight_report}" || -e "${preflight_log}" ]]; then
  echo "M4A_LAUNCH_REFUSED: target artifacts already exist" >&2
  exit 91
fi
for required in "${checkpoint}" "${base_config}" "${m4a_config}"; do
  if [[ ! -f "${required}" ]]; then
    echo "M4A_LAUNCH_REFUSED: missing ${required}" >&2
    exit 92
  fi
done

CUDA_VISIBLE_DEVICES=0 /opt/conda/bin/python -u \
  tools/preflight_japan4_deimv2_m4a_dysample_p5p4.py \
  --base-config "${base_config}" \
  --m4a-config "${m4a_config}" \
...
if [[ "${preflight_status}" -ne 0 ]]; then
  ...
  echo "M4A_LAUNCH_REFUSED: dynamic preflight failed" >&2
